...
public class VistaNBU extends JPanel implements IVistaNBU {
    ...
    private final Color C_SELECCION    = new Color(170, 205, 240);
    ...
    public VistaNBU() {
        ...
        configurarDobleClicReferencia(); // <-- Reparado y conectado
    ...
    public void setPresenter(NBUPresenter presenter) {
        ...
        txtBuscarNBU.addKeyListener(new java.awt.event.KeyAdapter() {
            @Override public void keyReleased(java.awt.event.KeyEvent e) {
                if (!actualizandoVista && !cargandoDatos) presenter.onBuscarNBU();
            }
        });
    ...
    @Override public int getIdDeterminacion(int f)    { return 0; }
    @Override public int getIndiceHijoSeleccionado()  { return grillaHijos.getSelectedRow(); }
    @Override public int getIndicePadreSeleccionado() { return grillaNBU.getSelectedRow(); }
    @Override public int getCantidadFilasPadre()      { return grillaNBU.getRowCount(); }
    ...
    public String pedirNombreNuevoComponente() {
        // FIX: Campo de texto con foco automático garantizado
        ...
        // FIX: Mensaje claro y con formato profesional usando HTML
        javax.swing.JLabel lblTip = new javax.swing.JLabel(
            ...
            + "Para crear un título o separador visual, escríbalo entre guiones.<br>"
    ...
    @Override public String getCodigoHijoFila(int f) {
        return grillaHijos.getModel().getValueAt(f, 0).toString();
    }
    @Override public String getUnidad(int f) {
        Object v = grillaHijos.getModel().getValueAt(f, 3); return v != null ? v.toString() : "";
    }
    @Override public String getReferencia(int f) {
        Object v = grillaHijos.getModel().getValueAt(f, 4); return v != null ? v.toString() : "";
    }
    @Override public String getCodigoPadreSeleccionado() {
        int f = grillaNBU.getSelectedRow();
        return f == -1 ? null : grillaNBU.getModel().getValueAt(f, 1).toString();
    }
    @Override public String getCodigoHijoSeleccionado() {
        int f = grillaHijos.getSelectedRow();
        return f == -1 ? null : grillaHijos.getModel().getValueAt(f, 0).toString();
    }
    @Override public String getNombreHijoFila(int f) {
        Object v = grillaHijos.getModel().getValueAt(f, 2); return v != null ? v.toString() : "";
    }
    @Override public String getCodigoPadreFila(int f) {
        if (f < 0 || f >= grillaNBU.getRowCount()) return "";
        Object v = grillaNBU.getModel().getValueAt(f, 1); return v != null ? v.toString() : "";
    }
    ...
    private void recalcularAlturaFila(int row) {
        ...
        for (int col = 0; col < grillaHijos.getColumnCount(); col++) {
            ...
            if (v != null && v.toString().contains(";")) {
                ...
                if (req > altura) altura = Math.min(req, 120);
            }
        }
    ...
    // ════════════════════════════════════════════════════════════════
    //  EDICIÓN CON DOBLE CLIC (JDialog Modal)
    // ════════════════════════════════════════════════════════════════
        ...
        grillaHijos.addMouseListener(new java.awt.event.MouseAdapter() {
            ...
            public void mouseClicked(java.awt.event.MouseEvent e) {
                if (e.getClickCount() == 2) {
                    ...
                    // FIX: Ya no preguntamos si es editable. Solo validamos la fila y la columna 4
                    if (row >= 0) {
                        ...
                        if(!esSubtitulo && col == 4) { // 4 = Valores de Referencia
    ...
    private void abrirEditorTextoLargo(int row, int col, String textoInicial) {
        ...
        // FIX: Instrucciones amigables, claras y en negrita
        JLabel lblSugerencia = new JLabel("<html><div style='font-family: Segoe UI; padding-bottom:5px;'>"
            + "💡 <b>¡Escriba prolijo!</b> Todo lo que ingrese aquí <b>saldrá tal cual en el informe final</b>.<br>"
        ...
        btnGuardar.addActionListener(e -> {
            ...
            if (nuevoTexto.endsWith(";")) nuevoTexto = nuevoTexto.substring(0, nuevoTexto.length() - 1);
        ...
        // FIX: Hacer foco automáticamente en el área de texto al abrir el JDialog
    ...
    private void aplicarEsteticaProfesional() {
        ...
        // 50/50
        gc.gridx = 0; gc.weightx = 0.50; gc.insets = new Insets(0, 0, 0, 8);
        pnlCuerpo.add(pnlTablaContainer, gc);
        gc.gridx = 1; gc.weightx = 0.50; gc.insets = new Insets(0, 8, 0, 0);
    ...
    private void mostrarAyudaRapida() {
        String msj = "<html><body style='width:380px;font-family:Segoe UI;color:#28323c;'>"
            ...
            + "Al vincular, escriba el nombre entre guiones: <b>--- TÍTULO ---</b><br>"
            + "<h3 style='color:#23a073;margin-top:15px;margin-bottom:5px;'>3. Reordenar</h3>"
            + "Seleccione una fila y use <b>▲ / ▼</b> para moverla. La selección se mantiene.</body></html>";
    ...
    private void initComponents() {
        ...
        grillaHijos.setModel(new DefaultTableModel(
            ...
            @Override public boolean isCellEditable(int r, int c) {
                // Ya no editamos directamente en la grilla la columna 4
                // Lo manejamos con el evento del doble clic
                if (c == 3) return true; // Unidad sí se puede editar directo si lo deseas
                return false;
            }
        });
