...
public class VistaAjustes extends javax.swing.JDialog implements IVistaAjustes {
    ...
    private boolean controladoreRegistrado = false;

    // ── Tipografía Equilibrada (Legible pero contenida) ───────────────
    private final Font F_SECTION = new Font("Segoe UI", Font.BOLD, 14);
    private final Font F_LABEL   = new Font("Segoe UI", Font.PLAIN, 13);
    private final Font F_CAMPO   = new Font("Segoe UI", Font.PLAIN, 14);
    private final Font F_BTN     = new Font("Segoe UI", Font.BOLD, 13);
    private final Font F_UB      = new Font("Segoe UI", Font.BOLD, 38);
    private final Font F_TAB     = new Font("Segoe UI", Font.BOLD, 14);
    ...
    public VistaAjustes(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        aplicarEstilo();
        poblarCombos();
    ...
    private void aplicarEstilo() {
        ...
        setPreferredSize(new Dimension(950, 650)); // Tamaño más compacto y manejable
        setResizable(true);
        ...
        // Password fields (Largo justo para contraseñas)
        ...
        // Botones principales (Armoniosos)
        ...
        lblUsuarioActual.setFont(new Font("Segoe UI", Font.BOLD, 16));
    ...
    public void setControlador(Controlador control) {
        if (controladoreRegistrado) return;
        controladoreRegistrado = true;
        this.controlador = control;

        btnActualizarClave.addActionListener(control);
        btnActualizarClave.setActionCommand(BTN_ACTUALIZAR_CLAVE);
        btnActualizarDatos.addActionListener(control);
        btnActualizarDatos.setActionCommand(BTN_ACTUALIZAR_DATOS);
        btnGuardarConfiguracion.addActionListener(control);
        btnGuardarConfiguracion.setActionCommand(BTN_GUARDAR_CONFIGURACION);
        btnGuardarUB.addActionListener(control);
        btnGuardarUB.setActionCommand(BTN_GUARDAR_UB);
    ...
    private void initComponents() {
        ...
        chkIncluirLogo       = new JCheckBox("Incluir logotipo en el informe");
        chkImprimirAutom     = new JCheckBox("Exportar PDF automáticamente al guardar");
        ...
        pnlHeaderGlobal.setBorder(new EmptyBorder(16, 25, 16, 25)); 

        JPanel pnlHdrIzq = new JPanel();
        pnlHdrIzq.setOpaque(false);
        pnlHdrIzq.setLayout(new BoxLayout(pnlHdrIzq, BoxLayout.Y_AXIS));

        JLabel lblHdrTag = new JLabel("SISTEMA BIOTEC — PANEL DE CONFIGURACIÓN");
        lblHdrTag.setFont(new Font("Segoe UI", Font.BOLD, 11));
        ...
        pnlHdrDer.add(Box.createVerticalStrut(4));
        ...
        gf.gridy = 1; gf.insets = new Insets(0,0,4,0);    cardClave.add(fldLabel("Contraseña actual"), gf);
        gf.gridy = 2; gf.insets = new Insets(0,0,16,0);   cardClave.add(txtClaveActual, gf);
        gf.gridy = 3; gf.insets = new Insets(0,0,4,0);    cardClave.add(fldLabel("Nueva contraseña"), gf);
        gf.gridy = 4; gf.insets = new Insets(0,0,16,0);   cardClave.add(txtNuevaClave, gf);
        gf.gridy = 5; gf.insets = new Insets(0,0,4,0);    cardClave.add(fldLabel("Confirmar nueva contraseña"), gf);
        gf.gridy = 6; gf.insets = new Insets(0,0,24,0);   cardClave.add(txtRepetirNuevaClave, gf);
        ...
        addCenteredCard(pnlSeguridad, cardClave, 30); // fill = NONE para no estirar infinitamente
        ...
        for (String[] lf : new String[][]{
                {"Nombre del laboratorio"}, {"Dirección física"},
                {"Ciudad / Localidad"}, {"Teléfono de contacto"},
                {"Bioquímico responsable"}, {"Matrícula profesional"}}) {
            gd.gridy = rd++; gd.insets = new Insets(0,0,3,0);
            cardDatos.add(fldLabel(lf[0]), gd);
            gd.gridy = rd++; gd.insets = new Insets(0,0,12,0);
        }
        ...
        JPanel cardRutas = card("RECURSOS Y RUTAS DEL SISTEMA");
        GridBagConstraints gr = new GridBagConstraints();
        gr.fill = GridBagConstraints.HORIZONTAL; gr.weightx = 1.0; gr.gridx = 0;
        int rr = 1;
        String[] labelsRutas = {"Imagen del logotipo", "Imagen de la firma", "Exportar informes PDF a", "Copias de seguridad en"};
        ...
        for (int i = 0; i < labelsRutas.length; i++) {
            gr.gridy = rr++; gr.gridwidth = 2; gr.insets = new Insets(0,0,3,0);
            ...
            gr.gridy = rr++; gr.gridwidth = 2; gr.insets = new Insets(0,0,14,0);
        ...
        gbs.insets = new Insets(15, 0, 0, 0);
        cardRutas.add(btnActualizarDatos, gbs);

        GridBagConstraints gcL = new GridBagConstraints();
        gcL.gridy = 0; gcL.weighty = 1.0; 
        gcL.fill = GridBagConstraints.HORIZONTAL; // Horizontal en lugar de BOTH
        gcL.anchor = GridBagConstraints.NORTH;    // Que se queden arriba si sobra espacio
        gcL.gridx = 0; gcL.weightx = 0.50; gcL.insets = new Insets(20, 20, 20, 10);
        pnlLaboratorio.add(cardDatos, gcL);
        gcL.gridx = 1; gcL.weightx = 0.50; gcL.insets = new Insets(20, 10, 20, 20);
        ...
        JPanel filaCombos = new JPanel(new GridLayout(1, 2, 25, 0));
        filaCombos.setOpaque(false);
        filaCombos.add(comboPanel("Tamaño de papel", cbxTamanoHoja));
        ...
        colImp.add(Box.createVerticalStrut(15));
        colImp.add(cardOpc);
        
        addCenteredCard(pnlImpresion, colImp, 30);

        // ════════════════════════════════════════════════════════════
        //  TAB 4 — ARANCELES
        // ════════════════════════════════════════════════════════════
        JPanel cardAran = card("VALOR DE LA UNIDAD BIOQUÍMICA (UB)");

        JLabel lblDesc = new JLabel("Define el valor monetario base para el cálculo automático del precio de los análisis.");
        ...
        gcDesc.insets = new Insets(5, 0, 25, 0);
        cardAran.add(lblDesc, gcDesc);

        JPanel filaUB = new JPanel(new FlowLayout(FlowLayout.CENTER, 20, 0));
        filaUB.setOpaque(false);

        JLabel lblSymbol = new JLabel("$");
        lblSymbol.setFont(new Font("Segoe UI", Font.BOLD, 52)); 
        ...
        gcUB.fill = GridBagConstraints.HORIZONTAL; gcUB.insets = new Insets(0, 0, 25, 0);
        ...
        addCenteredCard(pnlAranceles, cardAran, 50);
    ...
    private JPanel card(String titulo) {
        ...
        p.setBorder(BorderFactory.createCompoundBorder(
            new LineBorder(C_BORDE, 1, true),
            new EmptyBorder(25, 35, 25, 35) // Respiro interno armónico
        ));
    ...
    /** * Truco clave para evitar que las Cards se deformen: 
    ...
    private void addCenteredCard(JPanel tab, JComponent card, int vMargin) {
        ...
        g.fill = GridBagConstraints.NONE; // NO ESTIRAR INFINITAMENTE
        g.anchor = GridBagConstraints.CENTER; // MANTENER CENTRADO
        g.insets = new Insets(vMargin, 20, vMargin, 20);
        tab.add(card, g);
    }
