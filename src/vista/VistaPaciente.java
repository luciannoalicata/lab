...
public class VistaPaciente extends JPanel implements IVistaPaciente {
    ...
    public VistaPaciente() {
        ...
        grillaPacientes.getSelectionModel().addListSelectionListener(e -> {
            ...
            if (controlador != null && controlador.getUsuarioLogueado().getRol().equals("LECTOR")) {
                btnCargarResultados.setEnabled(false);
            } else {
                btnCargarResultados.setEnabled(haySeleccion);
            }
            btnVerHistorial.setEnabled(haySeleccion);
            if (!e.getValueIsAdjusting() && haySeleccion) {
                controlador.pacienteSeleccionado();
            }
        });
        ...
        txtBuscar.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            ...
            private void buscar() { if (controlador != null) controlador.buscarPacienteAutomatico(); }
        });
    }
        ...
        txtObraSocial.addKeyListener(new KeyAdapter() {
            ...
            public void keyReleased(KeyEvent e) {
                if (e.getKeyCode() == KeyEvent.VK_DOWN || e.getKeyCode() == KeyEvent.VK_UP
                        || e.getKeyCode() == KeyEvent.VK_ENTER || e.getKeyCode() == KeyEvent.VK_ESCAPE) return;
                String texto = txtObraSocial.getText().trim();
                if (texto.length() >= 1) controlador.buscarSugerenciasOS(texto);
    ...
    public void setControlador(Controlador control) {
        ...
        btnGuardarPaciente.addActionListener(control);
        btnEditarPaciente.addActionListener(control);
        btnCargarResultados.addActionListener(control);
        btnVerHistorial.addActionListener(control);
        btnVolver.addActionListener(control);
        
        btnGuardarPaciente.setActionCommand(BTN_GUARDAR_PACIENTE);
        btnEditarPaciente.setActionCommand(BTN_EDITAR_PACIENTE);
        btnCargarResultados.setActionCommand(BTN_CARGAR_RESULTADOS);
        btnVerHistorial.setActionCommand(BTN_VER_HISTORIAL);
        btnVolver.setActionCommand(BTN_VOLVER_VPAC);
    }
